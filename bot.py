...
async def do_open_case(cb: CallbackQuery, bot: Bot, key: str, x2: bool):
    ...
    # Update tasks ПОСЛЕ анимации — чтобы уведомления о заданиях шли после дропа
    ...
    # Только ПОСЛЕ показа дропа — обновляем задания (уведомления придут следующими сообщениями)
