...
CASES = {
    ...
    "grand": {"name": "💜 Grand Case", "price": 700, "desc": "Кейс Спонсора Grand News.\nСсылка на их соцсеть ( тгк: @GrandNewsQ )", "photo": GRAND_CASE_PHOTO, "items": [
    ...
    "neokzzless": {"name": "🟡 Neokzzless", "price": 700, "desc": "Кейс Спонсора Neokzzless.\nСсылка на их соцсеть ( тгк: @Neokzzless999 )", "photo": NEOKZZLESS_CASE_PHOTO, "items": [
...
async def handle_admin_photo(m: Message):
    ...
    var_name = f"{case_key.upper()}_CASE_PHOTO_ID"
    await m.answer(
        f"✅ Фото установлено (до перезапуска)!\n\n"
        f"📋 <b>Добавь в Railway Variables:</b>\n\n"
        f"Название: <code>{var_name}</code>\n"
        f"Значение: <code>{photo_id}</code>\n\n"
        f"Railway → проект → Variables → + New Variable"
    )
