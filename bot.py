...
CASES = {
    ...
    "grand": {"name": "💜 Grand Case", "price": 700, "desc": "Кейс Спонсора Grand News.\nСсылка на их соцсеть ( тгк: @GrandNewsQ )", "photo": "GRAND_CASE_PHOTO_ID", "items": [
...
async def handle_admin_photo(m: Message):
    ...
    await m.answer(f"✅ Фото для кейса '{case_key}' установлено!\nfile_id: <code>{photo_id}</code>\n\nСкопируй этот ID и вставь в CASES['{case_key}']['photo'] в коде для постоянного сохранения.")
...
async def open_case(cb: CallbackQuery, bot: Bot):
    ...
    if key.startswith("card_"):
        ...
        if photo_id and photo_id != "GRAND_CASE_PHOTO_ID":
            try:
