...
async def spend_coins(uid, amt):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    # Начисляем % ютуберу за трату
    await credit_youtuber_from_spend(uid, amt)
    return True
...
async def use_promo(uid, code):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if not promo:
            return None, "❌ Промокод не найден", 0, 0
        coins, max_uses, used_count, is_active, bonus_percent, youtuber_id, youtuber_percent = promo
        if not is_active:
            return None, "❌ Промокод уже неактивен", 0, 0
        if used_count >= max_uses:
            return None, "❌ Промокод исчерпан", 0, 0
        async with db.execute("SELECT 1 FROM promo_uses WHERE user_id=? AND code=?", (uid, code)) as c:
            already = await c.fetchone()
        if already:
            return None, "❌ Ты уже использовал этот промокод", 0, 0
        ...
        # Если промокод бонусный (bonus_percent > 0) — начисляем % от текущего баланса
        if bonus_percent > 0:
            async with db.execute("SELECT coins FROM users WHERE user_id=?", (uid,)) as c:
                row = await c.fetchone()
            cur_balance = row[0] if row else 0
            bonus = int(cur_balance * bonus_percent / 100)
            total_coins = coins + bonus
        else:
            bonus = 0

        await db.execute("UPDATE users SET coins=coins+? WHERE user_id=?", (total_coins, uid))

        # Привязываем промо к юзеру (для начисления % ютуберу с трат)
        if youtuber_id and youtuber_percent > 0:
            await db.execute(
                "INSERT OR REPLACE INTO user_active_promo (user_id,promo_code,youtuber_id,youtuber_percent) VALUES (?,?,?,?)",
                (uid, code, youtuber_id, youtuber_percent)
            )

        await db.commit()
        return total_coins, "ok", bonus_percent, bonus

async def credit_youtuber_from_spend(uid, spent_amount):
    """Начисляет % ютуберу при трате монет юзером (вызывать после spend_coins)."""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT youtuber_id,youtuber_percent FROM user_active_promo WHERE user_id=?", (uid,)
        ) as c:
            row = await c.fetchone()
        if not row:
            return
        youtuber_id, youtuber_percent = row
        earn = int(spent_amount * youtuber_percent / 100)
        if earn <= 0:
            return
        await db.execute(
            "UPDATE youtuber_earnings SET total_earned=total_earned+?,pending=pending+? WHERE youtuber_id=?",
            (earn, earn, youtuber_id)
        )
        await db.commit()
...
async def promo_activate(m: Message, state: FSMContext, bot: Bot):
    await state.clear()
    code = m.text.strip()
    coins, result, bonus_percent, bonus = await use_promo(m.from_user.id, code)
    if result != "ok":
    ...
    await check_achievements(m.from_user.id, bot)
    bonus_line = ""
    if bonus_percent > 0 and bonus > 0:
        bonus_line = f"\n🎁 Бонус <b>+{bonus_percent}%</b> к балансу: <b>+{bonus} монет</b>"
    await m.answer(
...
async def promo_admin(m: Message):
    ...
    if len(parts) < 2:
        await m.answer(
            ...
            "<b>Бонусный промокод (+% к балансу):</b>\n"
            "<code>/promo bonus КОД ПРОЦЕНТ МАКС_ИСПОЛЬЗОВАНИЙ</code>\n"
            "Пример: <code>/promo bonus BONUS20 20 100</code>\n"
            "(юзер введёт — получит +20% к своему балансу)\n\n"
            "<b>Промокод ютубера (% от трат юзера → ютуберу):</b>\n"
            "<code>/promo youtuber КОД ID_ЮТУБЕРА ПРОЦЕНТ_ЮТУБЕРУ МОНЕТЫ_ЮЗЕРУ МАКС</code>\n"
            "Пример: <code>/promo youtuber VASYA123 123456789 5 200 999</code>\n"
            "(юзер получает 200 монет, 5% его трат → ютуберу)\n\n"
    ...
    if action == "create":
        ...
        if ok:
            await m.answer(
                ...
                f"🎁 +{bonus_pct}% к балансу юзера при активации\n"
        ...
        # /promo youtuber КОД ID_ЮТУБЕРА ПРОЦЕНТ МОНЕТЫ_ЮЗЕРУ МАКС
        if len(parts) < 6:
            await m.answer("Формат: /promo youtuber КОД ID_ЮТУБЕРА ПРОЦЕНТ_ЮТУБЕРУ МОНЕТЫ_ЮЗЕРУ [МАКС]")
            return
        code = parts[2]
        try:
            yt_id = int(parts[3])
            yt_pct = int(parts[4])
            coins = int(parts[5])
            max_uses = int(parts[6]) if len(parts) > 6 else 9999
        except ValueError:
            await m.answer("❌ Неверный формат чисел")
            return
        ...
        ok = await create_promo(code, coins, max_uses, youtuber_id=yt_id, youtuber_percent=yt_pct, youtuber_username=yt_username)
        if ok:
            await m.answer(
                f"✅ Промокод ютубера <code>{code.upper()}</code> создан\n\n"
                f"👤 Ютубер: @{yt_username} (ID: {yt_id})\n"
                f"💰 Юзер получает: {coins} монет при активации\n"
                f"📊 Ютубер получает: {yt_pct}% от каждой траты юзера\n"
                f"👥 Макс. активаций: {max_uses}\n\n"
                f"Ютубер может проверить баланс командой /mypromo"
            )
        else:
            await m.answer("❌ Такой промокод уже существует")
