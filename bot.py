...
async def do_open_case(cb: CallbackQuery, bot: Bot, key: str, x2: bool):
    ...
    # Update tasks (до анимации — прогресс считается сразу)
    ...
    # Достижения проверяем ДО показа результата, но сохраняем список — бейдж покажем после
    ...
    # Бейдж достижений — показываем только если есть новые
