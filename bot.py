...
CASES = {
    ...
    "grand": {"name": "💎 Гранд кейс", "price": 1500,
    "photo": "AgACAgIAAxkBAAFLHOJqHAntBVF1VlkWolG5q5o6bE3XzAACGR1rG4Ys4Eg9UcQ_kEv6ZAEAAwIAA3kAAzsE",
    "items": [
        {"name": "Паласы",       "rarity": "rare",      "emoji": "🔵", "value": 600},
        {"name": "Миромаш",      "rarity": "rare",      "emoji": "🔵", "value": 700},
        {"name": "Неколез",      "rarity": "epic",      "emoji": "🟣", "value": 1200},
        {"name": "Оуджи",        "rarity": "epic",      "emoji": "🟣", "value": 1500},
        {"name": "Альмеро",      "rarity": "epic",      "emoji": "🟣", "value": 1800},
        {"name": "Красный орел", "rarity": "legendary", "emoji": "🟡", "value": 3500},
        {"name": "Гранд приз",   "rarity": "legendary", "emoji": "🟡", "value": 5000},
    ], "weights": [28, 25, 20, 13, 9, 3, 2]},
}
STARS = {
...
async def cmd_case(m: Message):
    ...
    for k, c in CASES.items():
        ...
        b.button(text=f"{enough} {c['name']} — {c['price']} монет", callback_data=f"open_{k}")
...
async def open_case(cb: CallbackQuery, bot: Bot):
    ...
    lines = [f"{i['emoji']} {i['name']} — {i['value']} 🪙" for i in c["items"]]
    enough = "✅" if u["coins"] >= c["price"] else "❌"
    text = (
        f"📦 <b>{c['name']}</b>\n"
        f"💰 Цена: <b>{c['price']} монет</b>  |  Баланс: <b>{u['coins']} монет</b> {enough}\n\n"
        f"<b>Возможные дропы:</b>\n" + "\n".join(lines)
    )
    b = InlineKeyboardBuilder()
    b.button(text=f"🎰 Открыть за {c['price']} монет", callback_data=f"open_normal_{key}")
    ...
    if photo_id:
        try:
            ...
            pass
        await bot.send_photo(cb.from_user.id, photo=photo_id, caption=text, reply_markup=b.as_markup())
    else:
...
async def do_open_case(cb: CallbackQuery, bot: Bot, key: str):
    c = CASES[key]
    u = await get_user(cb.from_user.id)
    if not u or u["coins"] < c["price"]:
        await cb.answer(f"❌ Нужно {c['price']} монет, у тебя {u['coins'] if u else 0}", show_alert=True)
        return
    if not await spend_coins(cb.from_user.id, c["price"]):
        await cb.answer("❌ Ошибка списания монет", show_alert=True)
        return

    item = random.choices(c["items"], weights=c["weights"], k=1)[0]
    await add_inv(cb.from_user.id, item)

    # Анимация — если сообщение с фото, удаляем и шлём новое текстовое
    photo_msg = bool(cb.message.photo) if cb.message else False
    if photo_msg:
        try:
            await cb.message.delete()
        except Exception:
            pass
        msg = await bot.send_message(cb.from_user.id, "🎰 ⬜⬜⬜⬜⬜")
    else:
    ...
    last_id = await get_last_inv_id(cb.from_user.id)

    b = InlineKeyboardBuilder()
    b.button(text="🔄 Ещё раз",              callback_data=f"open_{key}")
    b.button(text="📦 Кейсы",               callback_data="menu_cases")
    b.button(text=f"💰 Продать +{item['value']}", callback_data=f"sell_confirm_{last_id}")
    b.button(text="🔙 Меню",                callback_data="back_main")
    b.adjust(2, 1, 1)
    ...
    # Уведомления ПОСЛЕ показа результата
    ...
    if key == "grand":
        await update_task_progress(cb.from_user.id, "open_grand", 1, bot)
